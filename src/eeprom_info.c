...
int i2c_gpio_write_eeprom(uint addr, int alen, uchar *buffer, int len)
{
...
int i2c_gpio_read_eeprom(uint addr, int alen, uchar *buffer, int len)
{
    int shift;
    i2c_gpio_send_start();
    if(alen > 0) 
...
void set_kernel_env(int width, int height)
{
    ...
    if(AT24c02_eeprom.data.backlight[0]==0x04 && AT24c02_eeprom.data.backlight[1]==0x04)
    {
        ...
        sprintf(envprm,"console=ttymxc0,115200 init=/init %s %s %s androidboot.hardware=freescale cma=384M",videoprm,consoleprm,Backlightprm);
    }
    else
    {
        sprintf(envprm,"console=ttymxc0,115200 init=/init %s %s androidboot.hardware=freescale cma=384M",videoprm,consoleprm);
    }
